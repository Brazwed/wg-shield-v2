...
generate_wg_password_hash() {
    local password="$1"
    local raw_output raw_hash

    raw_output=$(docker run --rm ghcr.io/wg-easy/wg-easy wgpw "$password" 2>&1) || return 1

    raw_hash=$(printf '%s\n' "$raw_output" | sed -n "s/^PASSWORD_HASH=//p" | head -n1 | tr -d "'" | tr -d '\r')

    if [ -z "$raw_hash" ]; then
        err "${WG_PASSWORD_HASH_FAILED:-Failed to generate WG-Easy password hash}"
        return 1
    fi
...
write_wg_easy_env() {
    ...
    local escaped_hash
    escaped_hash=$(escape_for_compose_env "$password_hash")
...
install_comp() {
    ...
    if [ "$silent" = "true" ]; then
        ...
        if ! has_docker; then
            install_docker || { warn "${ERR_DOCKER_NOT_INSTALLED}"; return 1; }
        fi
        ...
        if [ "$comp" = "wg-easy" ]; then
            local wg_host wg_pass password_hash
            wg_host=$(curl -s4 ifconfig.me 2>/dev/null || echo "YOUR_IP")
            wg_pass=$(openssl rand -hex 8)
            password_hash=$(generate_wg_password_hash "$wg_pass") || { err "${WG_PASSWORD_HASH_FAILED:-Failed to generate password hash}"; return 1; }
            write_wg_easy_env "$dir" "$wg_host" "$wg_pass" "$password_hash"
        fi
